...
class _ClientesPageState extends State<ClientesPage> {
  ...
  Future<Null> getListaClientes() async {
    ...
    _listaClientes =
        await ClientesProvider.clientesProvider.obtenerListaClientes(prefs.code, prefs.ruta);
    setState(() {});
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        children: <Widget>[
          ...
          Expanded(child: _searchResult.length != 0 || controller.text.isNotEmpty ? _creaListaClientes(context, _searchResult) : _creaListaClientes(context, _listaClientes)),
        ],
      ),
        );
  }
  ...
  onSearchTextChanged(String text) async {
    ...
    _listaClientes.forEach((clientes) {
      if (clientes.razon.contains(text))
        _searchResult.add(clientes);
    });
 ...
 _creaCard(ClientesModel producto) {
