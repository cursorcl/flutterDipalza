...
- Nueva constante de ruta `AppRoutes.ultimasVentasCliente` — la pantalla se agrega directamente a `_pages` en `home.page.dart` (no necesita entrada en `AppRouter.generateRoute`, igual que `ClientesPage`/`ProductosPage`, que tampoco tienen route-case propio para su uso como tab — solo `clientesSeleccion`/`productosSeleccion` lo tienen).
